...
//		}
    ...
    // Обработка для BergHOFF по товарам  начало

    protected function getProductsBergHOFF(&$languages, $default_language_id, $product_fields, $exist_meta_title, $offset = null, $rows = null, $min_id = null, $max_id = null)
    {
        $sql = "SELECT ";
        $sql .= "  p.product_id,";
        $sql .= "  GROUP_CONCAT( DISTINCT CAST(pc.category_id AS CHAR(11)) SEPARATOR \",\" ) AS categories,";
    ...
    function populateProductsWorksheetBergHOFF(&$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset = null, $rows = null, &$min_id = null, &$max_id = null)
    {
        // get list of the field names, some are only available for certain OpenCart versions
        $query = $this->db->query("DESCRIBE `" . DB_PREFIX . "product`");
        $product_fields = array();
        ...
        $products = $this->getProductsBergHOFF($languages, $default_language_id, $product_fields, $exist_meta_title, $offset, $rows, $min_id, $max_id);

        $produxts_from_xml = $this->XmlParser();
        $produxts_from_xml = $produxts_from_xml['offers'];
        $len = count($products);
        $min_id = ($len > 0) ? $products[0]['product_id'] : 0;
        $max_id = ($len > 0) ? $products[$len - 1]['product_id'] : 0;
        foreach ($products as $row) {
    ...
    // Обработка для BergHOFF по товарам  конец
    ...
    public function downloadBergHOFF($export_type, $offset = null, $rows = null, $min_id = null, $max_id = null)
    {
        // we use our own error handler
        global $registry;
        $registry = $this->registry;
        ...
        try {
            ...
            switch ($export_type) {
                ...
                case 'p':
                    ...
                    $worksheet = $workbook->getActiveSheet();
                    $worksheet->setTitle('Products');
                    $this->populateProductsWorksheetBergHOFF($worksheet, $languages, $default_language_id, $price_format, $box_format, $weight_format, $text_format, $offset, $rows, $min_id, $max_id);
                    $worksheet->freezePaneByColumnAndRow(1, 2);
    ...
    public function XmlParser()
    {
        ...
        $xml_link = isset($_POST['xml_link']) ? $_POST['xml_link'] : null;

        if ($xml_link !== null) {

            // $xml = file_get_contents('http://b2b.berghoffworldwide.ru/catalog_xml_export'); // загрузка файла по ссылке
            $xml = file_get_contents(trim($xml_link)); // загрузка файла по ссылке
...
//Формирование массива данных
            foreach ($offers->getElementsByTagName('offer') as $offer) {
                ...
                $childs = array(
                    ...
                    'id' => $pid,
                    'available' => ($offer->getAttribute('available') == 'true') ? '7' : '5',
                    'inStock' => $offer->getAttribute('inStock'),
                    'param' => array(),
                    'picture' => array()
                );
                    ...
                    if (in_array($child->nodeName, $names)) {
                        ...
                        if ($child->hasChildNodes()) {
                            ...
                            foreach ($child->childNodes as $node) {
                                if (($node->nodeName == 'categoryId') && (!empty($node->nodeValue))) {
                                    /*   $parent_categories[] = array(

                                           'item' => $node->nodeValue
                                       );*/
                                    $parent_categories[] = $node->nodeValue;
                            ...
                            if (!empty($parent_categories)) {
                                ...
                                $childs[$child->nodeName] = implode(",", $parent_categories);
